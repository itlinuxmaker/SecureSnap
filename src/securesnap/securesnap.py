        ...
        for destination in config['backup']['destinations']:
            ...
            if backup_type == 'incremental':
                if incremental_count >= full_backup_interval or not os.path.exists(snapshot_file):
                    ...
                    tar_command = ["tar", "-cpf", "-", "-g", snapshot_file, "--acls", "--xattrs", "-P", "--no-dereference"]  # "-" outputs tar to stdout
                ...
                archive_name += f"full-{incremental_count}.tar"
                tar_command = ["tar", "-cpf", "-", "-g", snapshot_file, "--acls", "--xattrs", "-P", "--no-dereference"]
                write_log(log_file, f"Perform full backup for {destination['archive']} and create snapshot su file {snapshot_file}.")
